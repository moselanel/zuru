...
function ExperiencesContent({ domain }: { domain: string }) {
          ...
          {isLoading ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredExperiences.map((experience: any) => (
                <Link key={experience.id} href={`/preview/${domain}/experiences/${experience.slug}`}>
...
export default function PreviewExperiencesPage() {
  ...
  return (
    <TenantProvider tenant={tenantData}>
      <ExperiencesContent domain={tenant} />
    </TenantProvider>
  )
}
